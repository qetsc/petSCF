...
#    dist = np.linalg.norm(xyz-pivot,axis=1)
    dists   = xyz - pivot
    dists2  = np.sum(dists * dists,axis=1)
    return np.argsort(dists2)

def getMinDistanceIndex(xyz,pivot = 0):
    """
    Calculates dist2 and returns the index
    of min dist2 from a pivot point.
    """
    dists   = xyz - pivot
    dists2  = np.sum(dists * dists,axis=1)
...
def getTSPSortIndices(xyz,idx0=0):
    """
    Returns indices of xyz sorted to minimize
    distances of neighbors starting from idx0.
    TSP: Traveling salesman problem
    Notes
    -----
    If idx0 is looped over all xyz values, a brute 
    force solution for TPS can be achieved by checking
    the minimum of the sum of distances
    """
    n    = len(xyz)
    ...
    idx[0] = idx0
    for i in range(n-1):
        minidx = getMinDistanceIndex(xyz[mask,:],xyz[i])
        xyzidx = np.where(xyz==xyz[mask][minidx])[0][0]
        mask[xyzidx] = False 
        idx[i+1] = xyzidx
    return idx

def getSumOfNeighborDistances(xyz):
    """
    Returns the sum of neighbor distances.
    """
    tmps  = xyz[1:,:] - xyz[:-1,:]
    return np.sqrt(np.sum(tmps*tmps))

def getMaxOfNeighborDistances(xyz):
    """
    Returns the max of neighbor distances.
    """
    tmps  = xyz[1:,:] - xyz[:-1,:]
    dists2 = np.sum(tmps*tmps,axis=1)
    return np.sqrt(np.max(dists2))
...
def writeSXYZ(symbols, positions,filename='tmp.xyz',comment='generated by xyztools.py'):
    natoms = len(symbols)
    with open(filename,'w') as f:
        f.write("{0}\n".format(natoms))
        f.write(comment+"\n")
        for i in xrange(natoms):
            f.write("{0:s} {1:.14f} {2:.14f} {3:.14f} \n".format(symbols[i],positions[i,0],positions[i,1],positions[i,2]))
    return  
